...
def random_permutation(array_like):
	perm = deepcopy(array_like)
	for _ in array_like:
		ri1 = randint(0, len(array_like) - 1)
		ri2 = randint(0, len(array_like) - 1)
...
def plot_tsp(path, problem, str_ =""):
	...
	arrow(xi[-1], yi[-1], (xi[0] - xi[-1]), (yi[0] - yi[-1]), head_width=a_scale, color='g', length_includes_head=True)
	for i in range(0, len(x) - 1):
		arrow(xi[i], yi[i], (xi[i + 1] - xi[i]), (yi[i + 1] - yi[i]), head_width=a_scale, color='r', length_includes_head=True)
